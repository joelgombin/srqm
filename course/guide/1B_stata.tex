...
\newthought{You can find help} for Stata in the series of manuals and handbooks published by Stata Press. Appropriate references for this course are the ``Getting Started'' manuals, which cover the same basic operations as this guide.%
...
During class, you will often need to switch from a window to another one. This can be done from the keyboard: use \kbd{Cmd-`} (Mac) or \kbd{Alt-Tab} (Win). Have a look at the other shortcuts in the `Window' menu for more tricks: for example, on \OSX, \kbd{Cmd-1} switches to the Command window.%
