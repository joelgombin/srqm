...
\section{Essential data skills}
	...
	\newthought{Now load a dataset.} Stata can load example data with the \cmd{sysuse} and \cmd{webuse} commands, as we previously saw with the \code{lifeexp} dataset at p.~\pageref{lifeexp}. The course provides its own teaching datasets, which can be loaded by passing their disk files to the \cmd{use} command.%
		%
		\footnote{The \cmd{use} command can also copy data from a \URL. As soon as the path to your data includes spaces, you will need to enclose it in double quotes. Stata looks for \texttt{.dta} files by default, so the extension is not needed here because all course datasets are provided in Stata 9/10 format.}%

	All datasets are in the \texttt{data} folder and require that you specify that path to the \cmd{use} command to load correctly. The example below will load a course dataset and then replace it with another one:%
...
The \cmd{in} operator selects observations by their row number, which has limited use in most circumstances.%
	\index{Selectors!Observation ranges: \docmsg{in}} %
...
Then, you might also want to adjust \emph{variable labels}, which are the short pieces of text attached to each variable. Use the \cmd[la var]{label variable} command to change the variable label to something meaningful, e.g. mentioning the unit of measurement of the variable. This example renames and relabels the \texttt{lrscale} variable in the \ess:
...
\begin{description}
	...
	The example below shows the output of the \cmd{fre} command for a categorical variable from the \ess dataset.\pkg{fre} Household net income, which was originally measured in country-specific units, was aggregated to deciles for cross-country comparability:\\[1em]
...
\begin{description}
	...
	We use the \cmd{fre} command to diagnose the issue, passing the \coab{rows}{r}{fre} option in order to show only a few values instead of the full range from 15 to 99. Strangely enough, values 98 and 99 indicate missing data here:\\[1em]
...
As previously with the female dummy recode from p.~\pageref{female-dummy} we have used a logical statement with the \cmd{if} command, to operate a command only for a selection of observations. The one above says: ``apply (the \texttt{clonevar} command) only to observations for which (marital status) is strictly inferior to 9''. A list of logical statements is shown below:
	...
	\label{tab:logical-symbols}%
		\index{Logical statements}\index{Selectors!Logical conditions: \docmsg{if}}
	\end{table}
